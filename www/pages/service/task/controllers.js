...
.controller("TaskCtrl", function($scope, $ionicTabsDelegate, $state, $ionicHistory, ServiceService, $ionicScrollDelegate) {
       ...
       /* $scope.canSelectMore = false;*/
        ...
        ServiceService.GetMyOrderAll($scope.data).success(function(data) {
            ...
            if (data.Status === true && data.MyObject && data.MyObject.length > 0) {
                ...
                $scope.tasksFilter = $scope.tasks.filter(function (val) {
                    val.checked = false;
                    return val.IsFlag == 0;
                });
    ...
    $scope.selectMore = function () {
        $scope.canSelectMore = !$scope.canSelectMore;
    };

    $scope.doneMore = function () {
        if ($scope.canSelectMore == false) {
            $scope.showToast('请先点击多选按钮,选择事项');
            return false;
        }
        var idArry = $scope.tasksFilter.filter(function (val) {
            return val.checked === true;
        });
        var idValueArray = [];
        idArry.map(function (val) {
            idValueArray.push(val.Id)
        });
        var id = idValueArray.join(',');
        console.log(id);
        $scope.showLoad();
        ServiceService.CompleteOrder(id).success(function(data) {
            $scope.hideLoad();
            if (data.Status === true) {
