angular.module('app.controllers').controller('farmCtrl', function($scope, $http, $window, $interval, $timeout, $cordovaContacts, $ionicTabsDelegate, $ionicPopup, FarmService,$cordovaFileTransfer, $ionicSlideBoxDelegate, $cordovaDatePicker, $state, $ionicModal, InitService, QQLbsService, $ionicScrollDelegate, ServiceService, SearchService, $ionicLoading, $ionicPlatform, ForemanService, AnalyticsService, $cordovaGeolocation) {
	...
	$scope.showSingleFarmInfo = function () {
		...
		$ionicModal.fromTemplateUrl('./pages/farm/single-list.html', {
			...
			FarmService.GetOriginTrack($scope.CheckFarmSendInfo).success(function(data) {
    ...
    $scope.chooseItem = null;
    $scope.choose = function(Plains, subitem) {
    ...
    $scope.saveEntry = function(type) {
		/*权限!!!*/
        if(checkHasThisAuthority('NS','RL','create') == false){
            $scope.showToast('不具有添加农事入栏的权限!');
            return false;
        }
        $scope.entry.ImageName = $scope.photoImg;
        $scope.entry.TaskGps = $scope.gpsStr;
        if ($scope.showA) {
            if ($scope.entry.PCNum == "") {
                $scope.showToast("请输入入栏总数");
                return false;
            }
            $scope.showLoad();
            $scope.entry.planlst = $scope.chooseItem.PID + "^" + $scope.entry.PCNum + ";";
            FarmService.RLPlainAll($scope.entry).success(function(data) {
                $scope.hideLoad();
                if (data.Status === true) {
                    $scope.chooseItem.plannum = parseInt($scope.chooseItem.plannum) + parseInt($scope.entry.PCNum);
    ...
    /*-s 查看功能按钮*/
    $scope.farmShowArray = ['喂养', '免疫', '治疗', '死亡', '淘汰','入栏','转栏','任务'];
    ...
    var onData = function(buffer) {
        ...
        if(array_data.length < 20 && array_data.length > 10){
            ...
            if (notifitime % 2 === 0) {
                ...
                FarmService.GetPlainName(result).success(function (data) {
                    if (data.Status === true) {
                        $scope.sacnInfo = data.MyObject;
                    } else {
                        console.log('没有找出信息');
                    }
                }).then(function (data) {
        ...
        function setValue(){
            if ($scope.currentPage === "feed") {
                ...
                $scope.farmFeed.PenID = $scope.sacnInfo.PenId;
                $scope.farmFeed.PlainID = $scope.sacnInfo.PlainID;
                $scope.farmFeed.PenName = $scope.sacnInfo.PenName;
                $scope.farmFeed.PlainName = $scope.sacnInfo.PlainName;
                ...
                $scope.farmVaccination.PenID = $scope.sacnInfo.PenId;
                $scope.farmVaccination.PlainID = $scope.sacnInfo.PlainID;
                $scope.farmVaccination.PenName = $scope.sacnInfo.PenName;
                $scope.farmVaccination.PlainName = $scope.sacnInfo.PlainName;
            } else if ($scope.currentPage === "Cure") {
                $scope.farmCure.PNO = result;
                $scope.farmCure.PenID = $scope.sacnInfo.PenId;
                $scope.farmCure.PlainID = $scope.sacnInfo.PlainID;
                $scope.farmCure.PenName = $scope.sacnInfo.PenName;
                $scope.farmCure.PlainName = $scope.sacnInfo.PlainName;
            } else if ($scope.currentPage === "Death") {
                $scope.farmDeath.DeathObject = result;
                $scope.farmDeath.PenID = $scope.sacnInfo.PenId;
                $scope.farmDeath.PlainID = $scope.sacnInfo.PlainID;
                $scope.farmDeath.PenName = $scope.sacnInfo.PenName;
                $scope.farmDeath.PlainName = $scope.sacnInfo.PlainName;
